...
pkg_postinst() {
    if [ ! -d "${ROOT}"/var/run/${PN}d/ ]; then
        ...
        chown mysql:root ${ROOT}/var/run/${PN}d/ || die "Could not chown the directory"
    else
        ewarn "Directory is already created, but it needs permission"
        ewarn "Run the following: chown mysql:root ${ROOT}/var/run/mysql"
    fi
    ...
    if [ ! -f "${ROOT}"/var/run/${PN}d/${PN}d.err ] && [ -d "${ROOT}"/var/run/${PN}d/ ]; then
        ...
        chown mysql:root /var/run/${PN}d/${PN}d.err || die "Could not assign permissions"
    else
